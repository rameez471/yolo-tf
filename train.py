...
def _main():
    ...
    # try:
    #     model_body = load_model(settings.DEFAULT_MODEL_PATH,compile=False)
    # except:
    #     model_body = yolo_body(Input(shape=(None,None,3)),num_anchors // 3,num_classes)
    #     model_body.load_weights(settings.DEFAULT_MODEL_PATH)
    # else:
    #     assert model_body.layers[-1].output_shape[-1] == \
    #             num_anchors / len(model_body.output) * (num_classes + 5), 'Missmatch between model and given anchor and class type'
